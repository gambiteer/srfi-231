...
(define (array-for-each array)
  (interval-for-each (array-getter array)
                     (array-domain array)))
...
(define (array-every array)
  (%%interval-every (array-getter array)
                    (array-domain array)))

(define (array-any array)
  (%%interval-any (array-getter array)
                  (array-domain array)))
  ...
  (case-lambda
   ...
   ((A storage-class mutable?)
    (let* ((A
      ...
      (array-for-each (array-map array-assign! curried-result A))
      (if (not mutable?)
          (%%array-freeze! result)
          result)))))
